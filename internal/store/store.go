...
func (s *Store) InsertManagedIdentity(ctx context.Context, identity ManagedIdentity) error {
	_, err := s.pool.Exec(ctx, `INSERT INTO managed_identities (ziti_identity_id, identity_id, workload_id, identity_type, ziti_service_id, agent_id, environment_id) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
...
func (s *Store) ResolveIdentity(ctx context.Context, zitiIdentityID string) (ManagedIdentity, error) {
	row := s.pool.QueryRow(ctx, `SELECT identity_id, workload_id, identity_type, ziti_service_id, created_at, agent_id, environment_id FROM managed_identities WHERE ziti_identity_id = $1`, zitiIdentityID)
	identity := ManagedIdentity{ZitiIdentityID: zitiIdentityID}
	if err := row.Scan(&identity.IdentityID, &identity.WorkloadID, &identity.IdentityType, &identity.ZitiServiceID, &identity.CreatedAt, &identity.AgentID, &identity.EnvironmentID); err != nil {
...
func (s *Store) ResolveIdentityByIdentityID(ctx context.Context, identityID uuid.UUID) (ManagedIdentity, error) {
	row := s.pool.QueryRow(ctx, `SELECT ziti_identity_id, workload_id, identity_type, ziti_service_id, created_at, agent_id, environment_id FROM managed_identities WHERE identity_id = $1`, identityID)
	identity := ManagedIdentity{IdentityID: identityID}
	if err := row.Scan(&identity.ZitiIdentityID, &identity.WorkloadID, &identity.IdentityType, &identity.ZitiServiceID, &identity.CreatedAt, &identity.AgentID, &identity.EnvironmentID); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return ManagedIdentity{}, ErrManagedIdentityNotFound
		}
