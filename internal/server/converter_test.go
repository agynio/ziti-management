...
func TestFromProtoServiceType(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "egress gateway",
			input: zitimanagementv1.ServiceType_SERVICE_TYPE_EGRESS_GATEWAY,
			want:  store.ServiceTypeEgressGateway,
		},
		{
			name:    "unspecified",
	...
	tests := []struct {
		...
		},
		{
			...
			input: &zitimanagementv1.HostV1Config{
				...
				AllowedAddresses:  []string{" api.github.com "},
				AllowedPortRanges: []*zitimanagementv1.PortRange{{Low: 443, High: 443}},
			},
			want: &ziti.HostV1ConfigData{
				Protocol:          "tcp",
				Address:           "10.0.0.1",
				...
				AllowedProtocols:  []string{"tcp"},
				AllowedAddresses:  []string{"api.github.com"},
				AllowedPortRanges: []ziti.PortRangeData{{Low: 443, High: 443}},
			},
		},
		{
			name: "valid forwarded destination",
			input: &zitimanagementv1.HostV1Config{
				ForwardProtocol:   true,
				ForwardAddress:    true,
				ForwardPort:       true,
		...
		},
		{
			...
			wantErr: "allowed_addresses[0] is empty",
		},
		{
			name: "invalid allowed port range",
			input: &zitimanagementv1.HostV1Config{
				Protocol:          "tcp",
				Address:           "10.0.0.1",
				Port:              443,
				AllowedPortRanges: []*zitimanagementv1.PortRange{{Low: 443, High: 80}},
			},
			wantErr: "allowed_port_ranges[0] high must be >= low",
		},
		{
			name: "forward address requires allowed addresses",
			input: &zitimanagementv1.HostV1Config{
				Protocol:       "tcp",
				Port:           443,
				ForwardAddress: true,
			},
			wantErr: "allowed_addresses is required when forward_address is true",
		},
		{
